...
public class EnhancedEventAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public interface OnEventAdminActionListener {
        ...
        void onDownloadParticipants(Event event);
    }
    ...
    private final Map<String, Boolean> adminStatus;
    private OnEventAdminActionListener adminActionListener;
    
    public EnhancedEventAdapter(Context context) {
        this(context, null);
    }
    
    public EnhancedEventAdapter(Context context, String clubId) {
        ...
        this.adminStatus = new HashMap<>();
        
        // Check admin status only if clubId is provided
        if (clubId != null) {
            checkAdminStatus();
        }
    }
    ...
    private void checkAdminStatus() {
        ...
        clubsRef.child(clubId).child("adminId").addValueEventListener(new ValueEventListener() {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                adminStatus.put(clubId, false);
            }
        });
    }
    
    public void setAdminActionListener(OnEventAdminActionListener listener) {
    ...
    class ActiveEventViewHolder extends RecyclerView.ViewHolder {
        ...
        private final ImageView adminActionsButton;
        ...
        ActiveEventViewHolder(@NonNull View itemView) {
            ...
            adminActionsButton = itemView.findViewById(R.id.adminActionsButton);
        ...
        void bind(Event event) {
            ...
            // Check admin status - only show admin controls if we're in club view
            boolean isAdmin = false;
            if (clubId != null && clubId.equals(event.getClubId())) {
                isAdmin = adminStatus.getOrDefault(clubId, false);
            }
            
            adminActionsButton.setVisibility(isAdmin ? View.VISIBLE : View.GONE);
            if (isAdmin) {
                adminActionsButton.setOnClickListener(v -> showAdminMenu(v, event));
            }
        }
        ...
        private void handleRegistration(Event event) {
            ...
            if (isRegistered) {
                ...
                View dialogView = LayoutInflater.from(context).inflate(R.layout.dialog_phone_input, null);
                TextInputEditText phoneInput = dialogView.findViewById(R.id.phoneInput);
                
                new MaterialAlertDialogBuilder(context)
                    .setTitle("Enter Phone Number")
                    .setMessage("Please provide your phone number for event registration")
                    .setView(dialogView)
        ...
        private void showAdminMenu(View view, Event event) {
            PopupMenu popup = new PopupMenu(context, view);
            popup.getMenu().add(0, 1, 0, "Edit Event");
            popup.getMenu().add(0, 2, 0, "View Participants");
            popup.getMenu().add(0, 3, 0, "Download Participants");
            popup.getMenu().add(0, 4, 0, "Delete Event");

            popup.setOnMenuItemClickListener(item -> {
                if (adminActionListener == null) return false;

                switch (item.getItemId()) {
                    case 1:
                        adminActionListener.onEditEvent(event);
                        return true;
                    case 2:
                        adminActionListener.onViewParticipants(event);
                        return true;
                    case 3:
                        adminActionListener.onDownloadParticipants(event);
                        return true;
                    case 4:
                        adminActionListener.onDeleteEvent(event);
                        return true;
                    default:
                        return false;
                }
